...
#if defined(__MK20DX128__) || defined(__MK20DX256__) || defined (REGBOT)
...
void i2c_t3::begin_(struct i2cStruct* i2c, uint8_t bus, i2c_mode mode, uint8_t address1, uint8_t address2, i2c_pins pins, i2c_pullup pullup, i2c_rate rate)
{
    ...
    // Set rate and filter
        ...
        {                                              // ----  -------
        case I2C_RATE_100:  *(i2c->F) = 0x28; break;   // 113k    318
		case I2C_RATE_400:  *(i2c->F) = 0x19; break;   // 375k     96
		case I2C_RATE_1000: *(i2c->F) = 0x0A; break;   // 1.0M     36
        default:            *(i2c->F) = 0x28; break;   // 100k    318 (defaults to slowest)
        }
        *(i2c->FLT) = 3;
    #elif F_BUS == 48000000
        switch(rate)                                   // Freq  SCL Div
...
// return: 0=success, 1=data too long, 2=recv addr NACK, 3=recv data NACK, 4=other error, 5=timeout
