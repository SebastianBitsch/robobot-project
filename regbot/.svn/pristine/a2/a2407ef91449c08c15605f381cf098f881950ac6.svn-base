...
UServerPort::UServerPort()
{
  int i;
  //
  printf(" UServerPort::UServerPort() \n");
  ...
  serverAlive.now();
  lastClientNumber = -1;
  printf(" UServerPort::UServerPort() - end\n");
}
...
{ // running the server
  ...
  int lastErrNo = 0;
  //
  printf("# socket server loop started\n");
  while (not terminate)
...
//           updateVars();
      ...
      //
      if (result)
      { // listen is successful, now accept connections
        if (verboseMessages)
...
void UServerPort::print(const char * preStr)
{
  ...
  if (running)
  {
    printf(" running, has %d clients (rx loops %d) verbose %d\n",
      getActiveClientCnt(), recvLoops, verboseMessages);
    for (i = 0; i < clientCnt; i++)
    {
...
bool UServerPort::stop(bool andWait)
{
  ...
  for (i = 0; i < clientCnt; i++)
    client[i]->stopConnection(true, NULL);
  // return thread state
  return not running;
}
