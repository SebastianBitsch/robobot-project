...
CLIENT_REV = "$Id: urobot.py 1262 2021-01-14 09:43:31Z jcan $"
...
class URobot(object):
  ...
  def init(self):
    ...
    self.ctrlTurnRadius = UControlUnit("ctra", self.parent, "Turn radius")
    self.ctrlCurrent = UControlUnit("cmcu", self.parent, "Motor current (left and right)")
    ...
    self.ctrlTurnRadius.init()
    self.ctrlCurrent.init()
    ...
    if self.con.isOpen():
      ...
      if (n > 0):
        try:
          ...
          e = True
        if e:
          print("URobot::usbWrite - closing connection")
          self.con.close()
          self.info.thisRobot.gotRobotName = False
  ...
  def decodeCommand(self, got, n):
    ...
    if n > 3:
      ...
      if (self.mission.readUserMissionLine(got)):
        ...
        pass
      elif self.ctrlTurnRadius.fromString(gg):
        pass
      elif self.ctrlCurrent.fromString(gg):
        pass
  ...
  def timerUpdate(self):
    ...
    self.ctrlTurnRadius.timerUpdate()
    self.ctrlCurrent.timerUpdate()
    #
  ...
  def configurationFileSaveDef(self):
    ...
    if ret == QtGui.QMessageBox.Save:
      ...
      if (self.isConnected() or self.wifiConnected):
        ...
        time.sleep(waitTime)
        self.ctrlTurnRadius.requestDataFromRobot()
        self.ui.statusbar.showMessage("Getting ctrl data (turn radius)  ..", 500)
        time.sleep(waitTime)
        self.ctrlCurrent.requestDataFromRobot()
        self.ui.statusbar.showMessage("Getting ctrl data (cmcu)  ..", 500)
        time.sleep(waitTime)
  ...
  def configurationFileSave(self, filename, verbose):
    ...
    if True:
      ...
      self.ctrlTurnRadius.configurationFileSave(config)
      self.ctrlCurrent.configurationFileSave(config)
      # mission
  ...
  def configurationFileLoad(self, filename, bootload):
    ...
    try:
      ...
      self.ctrlTurnRadius.configurationFileLoad(config)
      self.ctrlCurrent.configurationFileLoad(config)
      #print("mission start")
      # mission
