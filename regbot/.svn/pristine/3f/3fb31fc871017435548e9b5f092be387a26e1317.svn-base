...
void lineSensorEnable(bool value)
{
  lineSensorOn = value;
  printf("# lineSensorEnable: set to %d\n", value);
  
  if (value)
  { // sample time in ms
    int T = int(SAMPLETIME * 1000);
    for (int i = 0; i < LineSensorCnt; i++)
      lineSensorRaw[i]->enable(T);
  }
  else
  { // disable to save CPU time
    for (int i = 0; i < LineSensorCnt; i++)
      lineSensorRaw[i]->disable();
  }
}
...
bool setLineSensor(const char * buf)
{
  ...
  { // is for the line sensor
    ...
    { // assumed white line
      char * p1 = (char *)&buf[4];
      used = true;
      lineSensorEnable(strtol(p1, &p1, 10));
