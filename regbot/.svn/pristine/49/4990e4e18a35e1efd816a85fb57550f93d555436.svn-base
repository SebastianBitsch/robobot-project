...
public:
   ...
   * \param configBuffer 2kbuffer to send, or set to NULL to use the just saved configuration.
   * Sends the configuration as byte hex code  */
  void stringConfigToUSB(const uint8_t * configBuffer, int configBufferLength);
  /** send configuration to USB
   * configBuffer assumet to be in place (config).
   * Sends the configuration as byte hex code  */
  void stringConfigToFile();
  /**
   * is stringbuffer in use, i.e. loaded from a hard-coaded configuration (non-robot specific) */
  bool forDemoRunOnly()
  {
    return stringConfig and not readAll;
  }
  
  /**
   * load EE configuration from file (regbot.ee) 
   * \param andToUsb sends string to USB - recoded back to string
   * \returns true if no error found */
  bool fileConfigLoad(bool andToUsb);
...
public:
  ...
  uint16_t readWord();
  /**
   * read all for configuration - including robot ID and calibration values
   * this is used with webots only, read from flash is always "readAll" */
  bool readAll = true;
  /**
...
protected:
   ...
   * \param buffer is the byte-oriented copy of flash memory,
   * \param configStr is human readable version of configuration.
   * \returns number of bytes put into buffer */
  int getHardConfigString(uint8_t * buffer, const char * configSrc);
...
public:
  #ifdef TEENSY35
  static const int maxEESize = 4096;
  #else
  static const int maxEESize = 2048;
  #endif
