...
# "$Rev: 1282 $"
#CLIENT_REV = "$Id: regbot.py 1282 2021-08-09 10:43:38Z jcan $"
...
class UMainWindow(QtGui.QMainWindow):
  def __init__(self, parent = None):
    ...
    self.usb_paint_space.setGeometry(QtCore.QRect(0, 0, 650, 300))
    ...
    self.wifi_paint_space.setGeometry(QtCore.QRect(0, 0, 650, 300))
    ...
    #
    p = self.ui.tab_servo.palette() # IR distance
    p.setColor(self.ui.tab_servo.backgroundRole(), QtCore.Qt.lightGray)
    self.ui.tab_servo.setPalette(p)
    self.ui.tab_servo.setAutoFillBackground(True)
    #
    p = self.ui.tab_joy.palette() # IR distance
    p.setColor(self.ui.tab_joy.backgroundRole(), QtCore.Qt.lightGray)
    self.ui.tab_joy.setPalette(p)
    self.ui.tab_joy.setAutoFillBackground(True)
    #
    ...
    # wifi
    p = self.ui.frame_43.palette()
    p.setColor(self.ui.frame_43.backgroundRole(), QtCore.Qt.lightGray)
    self.ui.frame_43.setPalette(p)
    self.ui.frame_43.setAutoFillBackground(True)
    #
    ...
    self.ui.actionJoy.triggered.connect(self.menuShowJoy)
    ...
    self.ui.pushButton_ctrlTurnRadius.clicked.connect(self.robot.ctrlTurnRadius.editControlValues)
    self.ui.pushButton_ctrlMotor.clicked.connect(self.robot.ctrlCurrent.editControlValues)
    # wifi
    ...
    self.ui.checkBox_servo1_use.clicked.connect(self.robot.servo.servo1use)
    self.ui.checkBox_servo2_use.clicked.connect(self.robot.servo.servo2use)
    self.ui.checkBox_servo3_use.clicked.connect(self.robot.servo.servo3use)
    self.ui.checkBox_servo4_use.clicked.connect(self.robot.servo.servo4use)
    self.ui.checkBox_servo5_use.clicked.connect(self.robot.servo.servo5use)
    print("Running ...")
    #
  def stop(self):
  ...
  def menuShowJoy(self):
    if (self.ui.actionJoy.isChecked()):
      self.ui.tabPages.insertTab(self.getIndex(11), self.ui.tab_joy, "Joy")
      self.ui.tabPages.setCurrentIndex(self.getIndex(11))
    else:
      t = self.ui.tabPages.indexOf(self.ui.tab_joy)
      if t >= 0:
  ...
  def menuShowAll(self):
    ...
    if not self.ui.actionJoy.isChecked():
  ...
  def tabPageFocusChanged(self):
    ...
    elif (tab == self.ui.tabPages.indexOf(self.ui.tab_joy)):
      # joy tab
      #self.robot.conWrite("joy get\n")
      print("# joy tab pressed")
      self.robot.currentTab = "joy"
    else:
