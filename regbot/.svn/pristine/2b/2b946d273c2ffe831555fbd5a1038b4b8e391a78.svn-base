...
EEConfig::EEConfig()
{
  ...
  stringConfig = false;
  config = NULL;
  hardConfig[0] = hardConfigFactoryReset;
  hardConfig[1] = hardConfigFollowWall;
  hardConfig[2] = hardConfigBalanceStep;
  hardConfig[3] = hardConfigWallWallAlign;
  hardConfig[4] = hardConfigSquare;
}
...
{ // reserve first 4 bytes for dword count
  ...
  else
  {
    push32(cnt);
    if (toUSB)
      snprintf(s, MSL, "# Send %lu config bytes (of 2048) to USB\r\n", cnt);
    else
      snprintf(s, MSL, "# Saved %lu bytes (of 2048) to EE-prom D\r\n", cnt);
  }
  configAddr = cnt;
  // tell user
  usb_send_str(s);
...
{ 
  ...
  if (cnt > (uint32_t)configAddr)
    ...
    usb_send_str("# no wifi info in eeProm\r\n");
