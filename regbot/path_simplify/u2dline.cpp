                      ...
                      int count)
{
  ...
  double a, b, l, cnt;
  //
  result = (count > 0) and (fabs(sX2 - sqr(sX)) > 1e-10);
  if (result)
  {
    cnt = double(count);
                      ...
                      int count)
{
  ...
  double a, b, l, cnt;
  //
  result = (count > 0) and (fabs(sY2 - sqr(sY)) > 1e-10);
  {
    cnt = double(count);
...
{ // find best line equation Ax + By + C = 0; from points in fx[i],fy[i], i=0..count-1.
  ...
  { // OK to calculate statistics
    ...
    cntD = double(count);
    // find y = a + bx, so B = 1; A=-b; C =-a;
    if (fabs(sX2/count - sqr(sX/count)) < 1e-10)
    {
      err -= 1; // all detections in same point in this plane
      v1 = 1e100;
    }
    ...
    // find e = a + bn, so A = 1; B = -b; C = -a
    if (fabs(sY2/count - sqr(sY/count)) < 1e-10)
    {
      err -= 1; // all detections in same point in this plane
      v2 = 1e100;
    }
