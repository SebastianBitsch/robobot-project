...
//     regTurnM[1] = pose[2];
  ...
  { // prepare next line
    ...
    printf("# UMissionThread::implementNewLine: (th=%d, lin=%d) start time=%d\n", threadNr, misLineNum + lineStartIndex, misStartTime);
...
//   printf("# UMissionThread::getLines: more-lines=%d, thread=%d, lines=%d\n", moreLine, threadNr, lineCnt);
...
{ // get token lines to console for debug
  ...
  snprintf(s, MSL, "#thread %d (%ld) %s\r", threadNr, strlen(reply), reply);
  usb_send_str(s);
  ...
  for (int i = 0; i < lineCnt; i++)
  {
    ...
    //usb_send_str(reply);
    snprintf(s, MSL, "#line %d (%ld) %s\r", i, strlen(reply), reply);
    usb_send_str(s);
...
int UMissionThread::toTokenString(char * bf, int bfCnt)
{
  ...
  if (activateFlag or deactivateFlag)
  {
    ...
    if (activateFlag)
    {
      snprintf(p1, bfCnt - n - 1, "%c%u", UMissionLine::MP_EVENT, activateFlag);
      n += strlen(p1);
      p1 = &bf[n];
    }
    if (deactivateFlag)
    { // add both ':' and stop-event(s)
      snprintf(p1, bfCnt - n - 1, ":%c%u", UMissionLine::MC_EVENT, deactivateFlag);
      n += strlen(p1);
    }
  }
...
//     printf("# UMissionThread::decodeToken: is thread\n");
...
//     printf("# UMissionThread::decodeToken: pre  decode thread %d line %d idx=%d isOK %d, %s\r\n", threadNr, lineCnt, idx, isOK, line);
    //     // debug
...
//       usb_send_str("# Line::testFinished -> 2 not finished\n");
    ...
    if (not finished and timeUse)
    {
      ...
      // printf("# UMissionLine::finished: start time=%d, now=%d (time=%d)\n", state->misStartTime, hbTimerCnt);
...
//   else
// not a good idea, as other threads may use the line sensor
//     lineSensorEnable(false);
...
//       usb_send_str("\n");
    ...
    if (eventSet)
    {
      snprintf(ms, bfCnt - n - 1, "%c%u", MP_EVENT, eventSet);
      n += strlen(ms);
      ms = &bf[n];
    }      
    if (headUse)
...
//     }
    ...
    if (eventMask)
    {
      snprintf(mc, bfCnt - n - 1, "%c%u", MC_EVENT, eventMask);
      n += strlen(mc);
      mc = &bf[n];
    }
    if (headEndUse)
...
//       usb_send_str("# servo velocity added");
  ...
  { // there might be a condition part
    ...
    { // not a valid operator
      snprintf(missionErrStr, missionErrStrMaxCnt, "invalid operator %s (pos %ld)", p3, p3 - buffer);
      err = true;
    }
//     p3 = strchr(p1, '=');
//     if (p3 == NULL)
...
//         usb_send_str("#found a last\n");
      ...
      { // error, just skip
        snprintf(missionErrStr, missionErrStrMaxCnt, "failed condition at %s (pos %ld)", p1, p1 - buffer);
        p1 = ++p3;
        err = true;
      }
      // remove white space
      ...
      if (strchr("=<>!", *p3) == NULL)
      { // not a valid operator
        snprintf(missionErrStr, missionErrStrMaxCnt, "invalid operator 2 %s (pos %ld)", p3, p3 - buffer);
        err = true;
      }
//       p3 = strchr(p1, '=');
//       if (p3 == NULL)
...
//  bool debugOut = false;
//   printf("# UMission::testFinished start - threads=%d, thredsMax=%d\n", threadsCnt, threadsMax);
  for (int i = 0; i < threadsCnt; i++)
  {
    if (threads[i].lineCnt == 0)
    { // no lines - goto next thread
      lineFinished[i] = true;
      continue;
    }
    //
    lineFinished[i] = threads[i].testFinished();
//     printf("# UMission::testFinished testFinished[%d]=%d\n", i, lineFinished[i]);
    // debug
//     if (lineFinished[i])
//     {
//       const int MSL=50;
...
//   int l = adr/32;
//   int c = adr - l*32;
//   printf("# UMission::eePromSaveMission: mission size %d saved at %x, line %d, pos %d\n", eeConfig.getAddr() - adr, adr, l, c);
...
//   char s[MSL];
  ...
  // debug
//   int a = eeConfig.getAddr() - 2;
//   int l = a/32;
//   int c = a-l*32;
//   printf("# UMission::eePromLoadMission: %d bytes (config addr = %x) line=%d, pos=%d\n", m, a, l, c);
...
//   usb_send_str(s);
  ...
  // is it a thread line
  const char * p1 = strstr(lineToAdd, "thread");
  if (p1 != NULL)
...
//   printf("# UMission::getLines get more=%d thread %d (threads = %d)\n", more, moreThread, threadsCnt);
...
//   printf("# UMission::getLines get more=%d thread %d (threads = %d) result = %d\n", more, moreThread, threadsCnt, result);
  return result;
}
