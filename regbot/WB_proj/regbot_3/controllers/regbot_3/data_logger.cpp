...
{ 
  ...
  if (logAllow and not logFull)
  {
    ...
    if (not logFull)
    {
      ...
      {
        float v[2] = {turnRate, steerWheelAngle};
...
void writeMission(int8_t * data, int row, char * p1, int maxLength)
{
  ...
  if (row < 0)
  {
    snprintf(p1, maxLength, "%% %2d %2d %2d %2d   (mission %d), state %d, entered (thread %d, line %d), events 0x%u (bit-flags)\r\n", 
              col, col+1, col+2, col+3, control.mission, v[0], vb[0], vb[1], vf[0]);
    col += 4;
  }
  else
  {
    snprintf(p1, maxLength, "%d %d %d %u ", v[0], vb[0], vb[1], vf[0]);
  }
}
...
void writeTurnrate(int8_t * data, int row, char * p1, int maxLength)
{
  ...
  if (row < 0)
  {
    snprintf(p1, maxLength, "%% %d %d Turnrate [r/s]: %.4f, steer angle [rad]: %.4f\r\n", col, col + 1, v[0], v[1]);
    col += 2;
  }
  else
    snprintf(p1, maxLength, "%.4f %.4f ", v[0], v[1]);
...
void writeEnc(int8_t * data, int row, char * p1, int maxLength)
{
  ...
  if (row < 0)
  {
    snprintf(p1, maxLength, "%% %2d %2d Encoder left, right: %u %u\r\n", col, col+1, v[0], v[1]);
    col += 2;
  }
  else
    snprintf(p1, maxLength, "%u %u ", v[0], v[1]);
}
...
void writeBaro(int8_t * data, int row, char * p1, int maxLength)
{
  ...
  if (row < 0)
  {
    snprintf(p1, maxLength, "%% %2d %2d %2d Barometer temp, pressure and height T, P, H: %.1f %d %.2f\r\n", col, col+1, col+2, *v / 10.0, *u, v[3]/100.0);
    col += 3;
  }
  else
    snprintf(p1, maxLength, "%.1f %d %.2f ", float(*v) / 10.0, *u, v[3]/100.0);
}
...
//   printf("# wrireBufferTo, row=%d, bufferPtr = %ld\n", row, bp - logBuffer);
  ...
  { // go througt all possible log items
    ...
    { // this log item is recorded
      ...
      { // reformat binary data as text to string from p1
        case LOG_TIME:  writeTime(&bp[logRowPos[item]], row, p1, MLL - n); 
          n += strlen(p1); 
          p1 = &logline[n]; 
          break;
...
//  logRowFlags[LOG_CTRL_VELR], (both controlled by left wheel controller)
  ...
  logRowCnt, logRowsCntMax, SAMPLETIME
  );  
  usb_send_str(reply);
}
