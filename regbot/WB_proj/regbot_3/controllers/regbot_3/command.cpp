...
#define REV_ID "$Id: command.cpp 1037 2019-03-12 11:34:21Z jcan $" 
...
UServerPort socketServer;

void initSocketServer()
{
  printf("# initSocketServer: starting socket server on 24001\n");
  socketServer.setPort(24001);
  socketServer.setVerbose(false);
  socketServer.setAllowConnections(true);
  socketServer.start();
  usleep(10000);
  printf("# initSocketServer: started socket server on 24001 (%d)\n", socketServer.isRunning());
}
...
//#ifdef WEBOT
void usb_serial_putchar(char a)
{
};
...
void usb_serial_write(const char * msg, int msgLen)
{
};
...
int usb_serial_write_buffer_free()
{
  return 1000;
}
...
void sendStatusLS()
{
  ...
  snprintf(reply, MRL, "ls %d %d  %d %d  %d %d  %d %d  %d %d  %d %d  %d %d  %d %d  %d %d  %d %d\r\n",
...
void testAddr(void)
{
  printf("# command.cpp::void testAddr(void) not implemented\n");
}
...
{
  UServerInMsg * msg;
  if (socketServer.getRxQueue()->isEmpty())
  {
    //printf("Queue is empty\n");
  }
  else
  { // message available
    msg = socketServer.getRxQueue()->skipToNextMessage(true);
//     msg->print("got");
    int n = strlen(msg->message);
    const int MSL = 100;
    char s[MSL];
    if (n >= 1)
    {
//       printf("# handleIncoming: got client (%d) %s\n", msg->client, msg->message);
      if (strncmp(msg->message, "alive", 5) == 0 or strncmp(msg->message, "<alive", 6) == 0)
      {  // just an alive signal - note
//         wifi.clientAlive[wifi.lastChannel] = hbTimerCnt;
        snprintf(s, MSL, "<alive last=\"%.5f\"/>\r\n", msg->rxTime.getDecSec());
        UServerClient * client;
        client = socketServer.getClient(msg->client);
        if (client != NULL)
        {
          if (client->isActive())
          {
            client->blockSend(s, strlen(s), 100);
          }
        }
        else
          printf("# handleIncoming: no such client (%d)\n", msg->client);
      }
      else
      { // so now, ready to parse
        parse_and_execute_command(msg->message, msg->client);
      }
    }
  }
...
//       snprintf(reply, MRL,"mca %g %g\r\n", motorCurrentA[0], motorCurrentA[1]);
      ...
      break;
    case 4:
      snprintf(reply, MRL,"enc 0x%x 0x%x\r\n", encoder[0], encoder[1]);
      break;
    case 5:
      sendWheelVelocity();
//       snprintf(reply, MRL,"wve %g %g\r\n", wheelVelocityEst[0], wheelVelocityEst[1]);
...
//                pose[0], pose[1], pose[2], distance, pose[3]);
    ...
    case 11: // motor 2 (1)
      ...
      { // allow local variables
        ...
        {
          snprintf(p1, MRL - n, " %u", encTime_cpu[m][i]);
          n += strlen(p1);
          p1 = &reply[n];
        }
        snprintf(p1, MRL - n, "\r\n");
...
//   printf("# parse_and_execute_command: got %s\n", buf);
...
//         case '2': wifi.sendStatusWiFiClients(); break;
    ...
    else if (buf[0] == 'h' || buf[0] == 'H')
    {
      ...
      usb_send_str(reply);
      snprintf(reply, MRL, "#   u0..u8       Status: u0:ver,u1:measure,u2:mission, u3:log,u4:robot,u5:heartbeat, u8:motorVA\r\n");
      usb_send_str(reply);
      ...
      usb_send_str(reply);
      snprintf(reply, MRL, "#   v0, v4       Status: v0:IR sensor data, v4: static timing info\r\n");
      usb_send_str(reply);
...
//       usb_send_str(reply);
    ...
    { // log controller values
      ...
      p2 = &buf[4];
      logRowFlags[LOG_CTRL_VELL] = strtol(p2, &p2, 10);
      logRowFlags[LOG_CTRL_VELR] = logRowFlags[LOG_CTRL_VELL];
...
//       usb_send_str(s);
    ...
    else if (strncmp(buf, "motv", 4) == 0)
    {
      ...
      if (fabs(m1) > 0.01 or fabs(m2) > 0.01)
        // this will take precedence to ant mission
    ...
    { // send to wifi serial connection
      printf("# command.cpp::parse_and_execute_command - wifi if not implemented\n");
    }
...
//         usb_send_str("# Got sub s p m\n");
    ...
    {
      snprintf(s, MSL, "#** unknown %ldc from %d command '%s'\r\n", strlen(buf), serChannel,  buf);
      usb_send_str(s);
...
bool usb_send_str(const char* str, bool blocking)
{
  bool send = false;
  // debug
//   printf("# usb_send_str: trying to send:%s", str);
  // debug end
  UServerClient * client;
  client = socketServer.getClient(requestingClient);
  if (client != NULL)
  {
    if (client->isActive())
    {
      send = client->blockSend(str, strlen(str), 100);
      if (not send)
        printf("# usb_send_str: failed send to %d msg:%s", requestingClient, str);
    }
  }
  else
    printf("# (%d) not send_to_usb: %s", requestingClient, str);
  return send;
}
