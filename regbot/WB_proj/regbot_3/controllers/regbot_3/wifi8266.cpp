...
void UWifi8266::sendStatusWiFi()
{
  const int MSL = 120;
  char s[MSL + 4];
  snprintf(s, MSL, "wfi %d %d %d %d  %d %d %d %d  %s %d %ld %s\r\n", not goingToSleep , setup, status, inSleep, 
...
void UWifi8266::enableWifi(bool useWifi)
{
  ...
  {
    if (wifi.inSleep)
      usb_send_str("# WIFI in sleep mode already\r\n");
    else
  ...
  switch (replyState)
  {
    ...
    case 2: // got an OK
      ...
      { // retry right away
        ...
        if (localEcho)
        {
          snprintf(s, MSL, "#\r\n# ... wifi took %u ms (to fail %d times, setup state=%d)\r\n", hbTimerCnt - wifiWait, wifiFailCnt, setup);
          usb_send_str(s);
        }
...
void UWifi8266::serialSetup()
{
    ...
    { // OK to proceed
      ...
      switch (setup)
      {
        ...
        case 2:
          ...
          // debug
          if (localEcho)
            usb_send_str("# Send MODE=3 (AT+CWMODE=3)\r\n");
          // debug end
          break;
        case 3:
          // get MAC (and IP)
          ...
          // debug
          if (localEcho)
            usb_send_str("# Send AT+CIFSR to get MAC\r\n");
          // debug end
          break;
        case 4:
  ...
  //         break;
        case 5:
          ...
          // debug
          if (localEcho)
            usb_send_str("# Send AT+CIPMUX=1 (multible connections)\r\n");
          // debug end
          break;
        case 6:
          ...
          // debug
          if (localEcho)
            usb_send_str("# Send CIPMUX=XX (XX=port number)\r\n");
          // debug end
          break;
        case 7:
          // get IP (and MAC)
          ...
          // debug
          if (localEcho)
            usb_send_str("# Send AT+CIFSR to get IP\r\n");
          // debug end
          break;
        case 8:
          ...
          if (wifiIP[3] != robotId)
          {
            snprintf(s, MSL, "# Robot ID=%d and IP4 LSb (%d.%d.%d.%d) is different\r\n", 
                    robotId, wifiIP[0], wifiIP[1], wifiIP[2], wifiIP[3]);
            usb_send_str(s);
          }
          setup++;
          break;
        ...
        case 14:
          ...
          { // close connection
            ...
            Serial1.write(s);
            replyType = WFI_NO_REPLY_YET;
            usb_send_str("# closeing client\r\n");
          }
          else
          {
            setup++;
  //           usb_send_str("# no client\r\n");
          }
          restartCnt = 0;
          break;
        case 15:
          ...
          if (not goingToSleep)
          {
            ...
            Serial1.write(s);
            replyType = WFI_NO_REPLY_YET; 
            usb_send_str("# resetting TCP server (setup state 15)\r\n");
            wifiFailCnt = 0;
            restartCnt++;
          }
          else
  ...
  //           setup = 1;
            if (localEcho and inSleep)
              usb_send_str("# is trying to restart socket server after sleep\r\n");
          }
          else
          {
            setup = 0;
            if (localEcho)
              usb_send_str("# sending wifi to sleep (reboot to restart)\r\n");
            enterSleepMode();
          }
          status = WFS_NONE;
  //         sendStatusWiFi();
...
// //       usb_send_str("# send request to 8266\r\n");
//     }
//   }
//   #endif
// }
...
//             usb_send_str("# wifi message length error\r\n");
//           else
//           {
//             memmove(p1, p2, mcnt);
//             // debug
...
//         // send status to USB
//         usb_send_str("# got a DISCONNECT from wifi (IP timeout?)\r\n");
//         requestingClient = -1;
//         sendStatusWiFi();
//       }
...
//         usb_send_str("# got a WIFI CONNECTED to wifi (new IP?)\r\n");
//         // send status to USB
//         requestingClient = -1;
//         wifiActive = true;
//         inSleep = false;
...
//       {
// //         status = WFS_GOT_IP;
//         usb_send_str("# got a WIFI GOT IP to wifi (new IP?)\r\n");
//         // send status to USB
//         requestingClient = -1;
//         sendStatusWiFi();
...
//           setup = 5;
//           usb_send_str("# WIFI repeat last setup - setting server mode and fetch IP and MAC\r\n");
//         }
//       }
//       else if (strstr(serRxBuf, "ready"))
//       { // received after a restart AT+RST reply
//         status = WFS_RST_READY;
//         usb_send_str("# WIFI restarted (AT+RST reply)\r\n");
...
//   {
//     wifiMsgLost++;
//     usb_send_str("# line too big for wifi (>98) - not send\r\n");
//   }
  if (clientActive[link] == WIFI_NOT)
  {  // client no longer alive - just drop it
    almostSend = true;
    wifiMsgLost++;
//     usb_send_str("# link not active - not send\n");
  } 
...
{ // send command to sleep for 60 seconds
  ...
  inSleep = true;
  // debug
  usb_send_str("# UWifi8266::enterSleepMode: setting wifi to sleep\n");
}
