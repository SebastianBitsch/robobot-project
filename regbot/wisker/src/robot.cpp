...
//     if (v1 == 0)
//       velIsZero = true;
//     else if (velIsZero and v1 > 0.01)
//     {
//       const int MSL = 200;
//       char s[MSL];
//       snprintf(s, MSL, "#%.6f, v1=%g, enc_ovl=%d, encPer=%lu, enc_cpu=%lu, dt_cpu %lu\n", 
//                time, v1,  encTimeOverload_cpu[0], encPeriod_cpu[0], 
//                encStartTime_cpu[0], dt_cpu);
//       usb_send_str(s);
//       velIsZero = false;
//     }
    // debug end
...
//   else
//   { // enc overload is set in timer interrupt
//     if (not encTimeOverload[0] and encPeriod10us[0] > 0)
//     { // time since last encoder pulse
//       int32_t ta = (int32_t)hb03us - (int32_t)encStartTime[0];
...
//         v1 = one_sec_in_10us/ta;
//       else
//         v1 = one_sec_in_10us/encPeriod10us[0];
