...
//       }
      ...
      {
        float val[3] = {float(control.chirpFrq), float(control.chirpAngle), control.chirpValue};
...
void writeChirp(int8_t * data, int row, char * p1, int maxLength)
{
  ...
  if (row < 0)
  {
    snprintf(p1, maxLength, "%% %2d %2d Chirp frequency =%g rad/s, phase=%g rad, value=%g\r\n", col, col+2, v[0], v[1], v[2]);
    col += 3;
  }
  else
  {
    snprintf(p1, maxLength, "%g %g %g ", v[0], v[1], v[2]);
  }
  return;
}
...
//   setLogSize(LOG_BAL_CTRL, 5, LOG_FLOAT);
  ...
  setLogSize(LOG_CHIRP, 3, LOG_FLOAT);
