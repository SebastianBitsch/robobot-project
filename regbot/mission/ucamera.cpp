...
{ 
  ...
  // 
  // ----- formats available use    $ v4l2-ctl -d/dev/video0 --list-formats
  // ----- OpenCV has the format coded like:
  // pixelFormat = V4L2_PIX_FMT_YUYV;
  // pixelFormat = V4L2_PIX_FMT_SRGGB8; // (GBRG) Bayer 8 bit
  pixelFormat = V4L2_PIX_FMT_SBGGR10;  // (BG10) Bayer coded 10bit per colour plane
  //pixelFormat = V4L2_PIX_FMT_SBGGR10P; // (pBAA) 10 bit bayer
  //pixelFormat = V4L2_PIX_FMT_SBGGR8; // (BA81) Bayer coded 8bit per colour plane
  // pixelFormat = V4L2_PIX_FMT_RGB24;
  //
  // ------ resolutions use         $ v4l2-ctl -d /dev/video0 --list-framesizes RGGB
  //   w = 2592;
  //   h = 1944;
  //
  w = 1640;
  ...
  { // try open camera with these settings
    ...
    if (cameraOpen)
    {
       ...
       * */
      setControl(0x00980914, 1); // flip h
      setControl(0x00980915, 1); // flip v
      //
      start_capturing();
      isOpen = true;
      // cam settings
...
//      setGain(100 /* 0=auto ?*/);
      //setExposure(-1); // 4--1183 (step 1), -1=auto
      /*
