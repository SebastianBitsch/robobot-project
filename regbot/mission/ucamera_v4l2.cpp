...
void UV4l2::process_image(void *p, int size, cv::Mat & imRGB)
{
  ...
  { // 8-bit Bayer (BA81)
    if (frame_number < 5)
      printf("# unpacking V4L2_PIX_FMT_SBGGR8 (8 bit Bayer BA81)\n");
...
//     cv::Size pxs(w,h);
  ...
  else if (pixelFormat == V4L2_PIX_FMT_SRGGB8)
  {
    if (frame_number < 5)
      printf("# unpacking V4L2_PIX_FMT_SRGGB8 (8 bit Bayer)\n");
    cv::Mat im(sz,CV_8UC1, (void*)p);  
    cv::demosaicing(im, imRGB, cv::COLOR_BayerBG2BGR, 3);
  }
  else
  {
    printf("# process_image - found unsupported format\n");
  }
  if (false)
...
void UV4l2::setControl(uint32_t ID, int value)
{
  //struct v4l2_queryctrl queryctrl;
  struct v4l2_control control;
  memset (&control, 0, sizeof (control));
  control.id = ID;
  control.value = value;
  int err = ioctl (fd, VIDIOC_S_CTRL, &control); // && errno != ERANGE;
  if (err == -1 and errno != EINVAL) 
  {
    printf("# 0x%x to %d failed\n", ID, value);
    perror ("# UV4l2::setControl failed ");
  }
  else
    printf("# UV4l2::setControl returned %d\n", err);
}  
...
void UV4l2::setExposure(int value)
{
  ...
  { // set exposure to value
    struct v4l2_control control;
    memset (&control, 0, sizeof (control));
    control.id = V4L2_CID_EXPOSURE; //  + V4L2_CID_EXPOSURE_AUTO;
    if (0 == ioctl (fd, VIDIOC_G_CTRL, &control)) 
    {
